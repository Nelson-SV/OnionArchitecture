...
export class UserClient {
    ...
    updateUserEmail(request: UpdateUserDto): Promise<UpdateUserResponseDto> {
        let url_ = this.baseUrl + "/api/UserController/UpdateUserEmail";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processUpdateUserEmail(_response);
        });
    }

    protected processUpdateUserEmail(response: Response): Promise<UpdateUserResponseDto> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
            let result200: any = null;
            result200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver) as UpdateUserResponseDto;
            return result200;
            });
        } else if (status !== 200 && status !== 204) {
            return response.text().then((_responseText) => {
        ...
        return Promise.resolve<UpdateUserResponseDto>(null as any);
    }

    deleteUser(userId: string): Promise<FileResponse> {
        let url_ = this.baseUrl + "/api/UserController/DeleteUser";
        url_ = url_.replace(/[?&]$/, "");
